# joyvend integrations — automatic capture

joyvend deliberately runs no LLM: the agent does the reasoning. Its one weak point is
...
- **Capture** — a per-turn hook calls `joyvend capture`, logging each raw turn as a
...
| **Any shell-driven agent** | [`generic/joyvend-wrap.sh`](generic/joyvend-wrap.sh) — captures each stdin turn |

All recipes are non-fatal by design: if joyvend is stopped, capture is silently skipped and
the turn proceeds normally. They require `joyvend` on `PATH` (a running server) and `jq`.
